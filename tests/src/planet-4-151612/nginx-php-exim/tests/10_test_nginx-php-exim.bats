...
@test "container starts" {
  run start_docker_compose "${compose_file}" "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
}

@test "container responds on port 80 with status 200" {
  run run_test_http_response_code 200 "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
}

@test "container responds on port 443 with status 200" {
  run run_test_http_response_code 200 "${ENDPOINT_HTTPS}"
  [[ $status -eq 0 ]]
}

@test "http response contains PHP 7 version string" {
  run run_test_http_response_grep "PHP Version 7.[0-9]*.[0-9]*" "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
}

@test "environment variable set correctly: PHP_MEMORY_LIMIT=${PHP_MEMORY_LIMIT}" {
  run run_test_http_response_grep "memory_limit.*${PHP_MEMORY_LIMIT}" "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
}

@test "environment variable set correctly: UPLOAD_MAX_SIZE=${UPLOAD_MAX_SIZE}" {
  run run_test_http_response_grep "upload_max_filesize.*${UPLOAD_MAX_SIZE}" "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
  run run_test_http_response_grep "post_max_size.*${UPLOAD_MAX_SIZE}" "${ENDPOINT_HTTP}"
  [[ $status -eq 0 ]]
}
